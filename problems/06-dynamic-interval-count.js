...
function dynamicIntervalCount(cb, delay, amount) {
  // Your code here
  //1. set a variable for an interval that accepts cb, delay, and amount(optional)
  let count = 0;
  let interval = setInterval(() => {
    cb();
    count++;
    //2. if the interval has an amount, clear the interval when the interval has been repeated those amount of times
     if (count === amount && amount !== undefined) {
      clearInterval(interval);
    }

  }, delay, amount)

  //3. if there is no amount set, then the interval will not be cleared and will return the Timeout object
  return interval;
